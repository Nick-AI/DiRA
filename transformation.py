...
class GaussianBlur(object):
    ...
    def __call__(self, x):
        ...
        x = torch.clone(x).numpy()
        x = scipy.ndimage.gaussian_filter(x, sigma=sigma, order=0)
...
class Custom_Distoration(object):
    ...
    def __call__(self, org_img):
        ...
        if r <= 0.3:  # cut-out
            cnt = 10
            while cnt > 0:
                block_noise_size_x, block_noise_size_y = random.randint(
                    10, 70), random.randint(10, 70)
                noise_x, noise_y = random.randint(
                    3, self.input_rows - block_noise_size_x - 3), random.randint(3, self.input_cols - block_noise_size_y - 3)
                org_img[:, noise_x:noise_x + block_noise_size_x,
            ...
            cnt = 10
            while cnt > 0:
                block_noise_size_x, block_noise_size_y = self.input_rows - \
                    random.randint(50, 70), self.input_cols - \
            ...
            while cnt > 0:
                while True:
                    ...
                    noise_x1, noise_y1 = random.randint(
                        3, self.input_rows - block_noise_size_x - 3), random.randint(3, self.input_cols - block_noise_size_y - 3)
                    noise_x2, noise_y2 = random.randint(
                        3, self.input_rows - block_noise_size_x - 3), random.randint(3, self.input_cols - block_noise_size_y - 3)
                ...
                org_img[:, noise_x1:noise_x1 + block_noise_size_x,
                        noise_y1:noise_y1 + block_noise_size_y] = image_temp[:, noise_x2:noise_x2 + block_noise_size_x,
